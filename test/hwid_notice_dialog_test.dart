...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const ValueKey('auto-check-notice-ok')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'HWID notice dialog shows in Russian when Russian locale is active',
    (tester) async {
      final base = await MemoryAppSettingsStore().loadState();
      final store = MemoryAppSettingsStore(
        base.copyWith(
  ...
  testWidgets(
    'Auto-check notice dialog shows directly when HWID was already acknowledged',
    (tester) async {
      final base = await MemoryAppSettingsStore().loadState();
      final store = MemoryAppSettingsStore(
        base.copyWith(
