...
class AppLocalizationsRu extends AppLocalizations {
  ...
  String get aboutDocumentationSubtitle =>
      'Пошаговые инструкции по подпискам, VPN, маршрутизации и диагностике.';

  @override
  String get documentationPageTitle => 'Справочник Etonify';

  @override
  String get documentationPageSubtitle =>
      'Пошаговая справка по подключению, подпискам, маршрутизации, безопасности и диагностике. Доступна без интернета.';
  ...
  @override
  String get documentationGroupHelp => 'Диагностика и помощь';
  ...
  String get documentationQuickStartBody =>
      '1. Добавьте подписку, файл, QR-код или отдельный сервер.\n2. Откройте список серверов и запустите проверку.\n3. Выберите сервер вручную или используйте Lowest.\n4. Нажмите кнопку подключения и подтвердите системный запрос Android.\n5. Если соединение не работает, сначала проверьте другой сервер, затем откройте логи.';

  @override
  String get documentationWhatTitle => 'Что такое Etonify';

  @override
  String get documentationWhatBody =>
      'Etonify — VPN-клиент для Android 8.0 и новее на базе sing-box. Клиент не продаёт и не выдаёт серверы: подписку, профиль или отдельный ключ вы получаете у своего провайдера и добавляете самостоятельно. Настройки, подписки и логи хранятся на устройстве.';
  ...
  String get documentationModesBody =>
      'VPN TUN создаёт системный VPN Android и может обрабатывать трафик приложений на телефоне.\n\nЛокальный HTTP/SOCKS-прокси предназначен для приложений и устройств, где адрес прокси задаётся вручную. Сам по себе он не перенаправляет весь трафик телефона и не управляет раздельной маршрутизацией приложений.\n\nВ режиме VPN локальный прокси можно включить дополнительно. Для доступа из локальной сети нужны показанные в Etonify адрес, порт, логин и пароль.';
  ...
  String get documentationProtocolsBody =>
      'Etonify импортирует ссылки, QR-коды, конфигурации sing-box и Xray, WireGuard-файлы и поддерживаемые ссылки Happ.\n\nОсновные протоколы: VLESS, VMess, Trojan, Shadowsocks, ShadowsocksR, Hysteria, Hysteria2, TUIC, WireGuard, AnyTLS, Naive, HTTP и SOCKS. Поддержка редкого расширения зависит от формата профиля и версии встроенного ядра. Успешный импорт означает, что конфигурация распознана, но не гарантирует доступность сервера.';
  ...
  String get documentationChainsBody =>
      'Цепочка последовательно использует первый прокси и выходной прокси. Оба узла должны работать; задержка обычно становится выше.\n\nЦепочка не является автоматическим резервированием: если один узел недоступен, вся цепочка перестаёт работать. Lowest проверяет и выбирает обычные серверы, но не оценивает цепочку как единый маршрут.';
  ...
  String get documentationSubscriptionsBody =>
      'Профиль объединяет название, источники подписки, серверы и последний выбранный сервер. В одном профиле может быть несколько URL — при обновлении они остаются внутри этого профиля.\n\nДобавить данные можно из буфера обмена, файла, QR-кода или вручную. Автообновление меняет содержимое подписки по расписанию, но не включает VPN. Если источник возвращает пустой ответ, HTML или ошибку HTTP, Etonify покажет причину и сохранит предыдущие рабочие данные.';

  @override
  String get documentationChecksTitle => 'Серверы и URLTest';
  ...
  String get documentationBackgroundBody =>
      'Пока VPN включён, Android показывает служебное уведомление Etonify. В нём доступны сервер, задержка, трафик и остановка VPN. Вид трафика и частоту обновления можно изменить в общих настройках.\n\nВыключение экрана и закрытие интерфейса не должны останавливать VPN. Однако принудительная остановка приложения в системных настройках запрещает Android запускать сервис до следующего ручного открытия Etonify. На прошивках с жёстким энергосбережением может понадобиться разрешить фоновую работу и автозапуск.';
  ...
  String get documentationRoutingBody =>
      'Раздельная маршрутизация работает только в режиме VPN TUN. Режим «Через VPN» направляет в туннель только выбранные приложения. Режим «Вне VPN» оставляет выбранные приложения на прямом подключении. Android Always-on VPN с блокировкой соединений может запретить прямой доступ.\n\nMixed использует системный стек Android для TCP и gVisor для UDP. System обрабатывает TCP и UDP системным стеком Android. gVisor обрабатывает оба протокола в пространстве пользователя. Универсально лучшего варианта нет: оставьте Mixed и меняйте стек только при подтверждённой проблеме совместимости.';
  ...
  String get documentationTrafficRulesBody =>
      'Правила трафика маршрутизируют домены и IP-диапазоны внутри VPN: например, российские сервисы напрямую, а нейросети через прокси. Одновременно можно включить только один готовый пресет.\n\nРаздельная маршрутизация и правила трафика — разные уровни. Сначала Android определяет, попадёт ли приложение в TUN, затем правило выбирает маршрут его соединения. Файлы правил работают локально после подготовки.';
  ...
  String get documentationDnsBody =>
      'Etonify поддерживает DNS устройства, UDP, TCP, DoT и DoH. Для обычного DNS можно указать IP, например 1.1.1.1 или 8.8.8.8, и выбрать UDP или TCP. Для DoH и DoT по доменному имени клиент подготавливает bootstrap-разрешение адреса.\n\nDNS «Напрямую» использует прямой маршрут, DNS «Через прокси» — выбранный прокси. Эти маршруты должны соответствовать правилам трафика. FakeIP находится в экспериментальных настройках и может быть несовместим с отдельными приложениями.';
  ...
  String get documentationRuleFilesBody =>
      'Пресеты используют локальные файлы .srs с доменами и IP-диапазонами. После подготовки они работают без интернета. Обновление безопасно заменяет файлы только после успешной загрузки; при ошибке остаётся предыдущая версия.\n\nБлокировка рекламы использует отдельный локальный фильтр AdGuard. После скачивания его можно включать и обновлять в разделе маршрутизации. Применение новых файлов может потребовать перезапуска VPN.';
  ...
  String get documentationSecurityBody =>
      'По умолчанию Etonify проверяет TLS-сертификаты прокси-серверов и HTTPS-подписок. В разделе «Безопасность» проверки можно отключить отдельно для прокси и подписок.\n\nРазрешайте недоверенные сертификаты только для известного источника: самоподписанный, просроченный или подменённый сертификат нельзя отличить без дополнительной проверки. Отключение проверки повышает риск перехвата ключей и трафика. Обычный HTTP для удалённых подписок не рекомендуется и ограничен локальными адресами.';

  @override
  String get documentationUpdatesTitle => 'Обновления и сетевой маршрут';

  @override
  String get documentationUpdatesBody =>
      'Подписки, георесурсы, фильтр рекламы и обновления клиента загружаются через доступный сетевой маршрут. При работающем VPN Etonify использует подходящий маршрут через туннель, а при неудаче пробует физическую Wi‑Fi или мобильную сеть. Если системный VPN принадлежит другому приложению, запрос проходит по правилам этого VPN.\n\nДля загрузок действуют таймауты и проверка размера. APK дополнительно проверяется перед передачей системному установщику Android.';
  ...
  String get documentationBackupBody =>
      'Экспорт настроек сохраняет параметры клиента без подписок и VPN-ключей. Экспорт подписок сохраняет профили, серверы и выбранные значения.\n\nИспользуйте экспорт с паролем: незашифрованный файл содержит ключи доступа в открытом виде. Пароль резервной копии не восстанавливается. Перед крупным обновлением или сбросом клиента создайте новую зашифрованную копию и храните её отдельно от телефона.';
  ...
  String get documentationExperimentalBody =>
      'FakeIP, фрагментация TLS, TCP Fast Open, TCP MultiPath и мультиплексирование меняют сетевое поведение и подходят не каждой сети или конфигурации. Для обычной подписки они не обязательны.\n\nМеняйте по одному параметру и проверяйте тот же сервер на той же сети. Не заменяйте SNI, ALPN и параметры QUIC значениями «на глаз». Если соединение стало нестабильным, верните изменённую настройку к значению по умолчанию.';

  @override
  String get documentationDiagnosticsTitle => 'Ресурсы, логи и версии';

  @override
  String get documentationDiagnosticsBody =>
      'Раздел «Ресурсы и отладка» показывает состояние VPN-сервиса, версии клиента и ядра, а также память процесса. PSS, RSS, Private Dirty, Swap, heap, code и graphics — разные представления памяти; складывать их нельзя.\n\nИзмеритель ресурсов записывает изменения памяти и нагрузки во времени. Логи содержат этапы сборки конфига, запуска ядра, смены сети и URLTest. Экспортируйте их сразу после воспроизведения проблемы и проверьте файл перед отправкой.';
  ...
  String get documentationLimitsBody =>
      'Etonify выпускается только для Android и не содержит бесплатных VPN-серверов. Работа зависит от конфигурации провайдера, доступности сервера, DNS, оператора связи и прошивки устройства.\n\nУспешный URLTest не гарантирует доступ ко всем сайтам: тест проверяет один адрес. Большие подписки, полные проверки тысяч серверов и частые обновления временно увеличивают расход памяти, процессора, батареи и трафика.';

  @override
  String get documentationSupportTitle => 'Нужна помощь?';

  @override
  String get documentationSupportBody =>
      '1. Повторите проблему, изменяя только одну настройку за раз.\n2. Запишите режим подключения, выбранный сервер и тип сети.\n3. Укажите модель устройства, версию Android, Etonify и ядра.\n4. Опишите точные шаги и ожидаемый результат.\n5. Сразу экспортируйте логи и удалите из отчёта приватные ключи.\n\nОтчёт можно отправить в чат поддержки: https://t.me/etonify?direct';
  ...
  String get diagnosticsSubtitle =>
      'Память процесса, состояние ядра и инструменты для поиска проблем.';
  ...
  String get updatesSubtitle =>
      'Проверка выпусков на GitHub и загрузка APK для этого устройства.';
